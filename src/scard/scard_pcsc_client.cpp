...
scard_pcsc_client::scard_pcsc_client(const std::string& path,
    std::size_t buffer_size_hint)
    : _socket(0), _buffer(buffer_size_hint)
{
    // create socket
    ...
    strncpy(_address.sun_path, path.c_str(), sizeof(_address.sun_path) - 1);
...
void scard_pcsc_client::connect()
{
    ...
    LOG(LOG_INFO,
        "scard_pcsc_client::connect: "
        "client not connected; connecting..."
    );

    ret = ::connect(_socket.fd(),
        reinterpret_cast<struct sockaddr *>(&_address), sizeof(_address));
    if (ret < 0) 
    { 
...
std::size_t scard_pcsc_client::send(uint32_t io_control_code, bytes_view data)
{
    ...
    LOG(LOG_DEBUG, "scard_pcsc_client::send: "
...
bytes_view scard_pcsc_client::receive(uint32_t io_control_code)
{
    int ret;
    ...
    LOG(LOG_DEBUG, "scard_pcsc_client::receive: "
