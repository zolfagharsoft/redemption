...
public:
    ///
    scard_pack_call_connect(SCARDCONTEXT hContext, const char *szReader,
...
public:
    ...
    ///
    const scard_pack_pci & ioRecvPci() const;
...
#define SCARD_PACK_CHECK_STREAM(stream, packable, eid) \
    check_stream((stream), (packable), __FILE__, __FUNCTION__, __LINE__, (eid));

///
void check_stream(const InStream &stream,
    const scard_pack_return *return_, const char *file,
    const char *function, int line, error_type eid);

///
void check_stream(const OutStream &stream,
