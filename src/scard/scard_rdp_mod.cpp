...
#include "keyboard/keymap2.hpp" // FIXME
...
SmartcardMod::SmartcardMod(
    ...
    , vars(vars)
{
    // instantiate smartcard manager
    this->manager = std::make_unique<scard_manager>(
        sesman, front, time_base, events
    );
    
    this->screen.set_wh(front_width, front_height);
    if (vars.get<cfg::globals::authentication_timeout>().count()) {
        LOG(LOG_INFO, "SmartcardMod: Ending session in %u seconds",
...
void SmartcardMod::rdp_input_invalidate(Rect r)
{
...
void SmartcardMod::rdp_input_mouse(int device_flags, int x, int y, Keymap2 * keymap)
{
...
void SmartcardMod::rdp_input_scancode(long param1, long param2, long param3, long param4, Keymap2 * keymap)
{
...
//            if ((param1 == 56) && (param3 == (SlowPath::KBDFLAGS_DOWN | SlowPath::KBDFLAGS_RELEASE))) {
            if ((param1 == 56) && (param3 & SlowPath::KBDFLAGS_RELEASE)) {
                ...
                LOG(LOG_INFO, "SmartcardMod::rdp_input_scancode: Close by user (Alt+F4)");
...
void SmartcardMod::refresh(Rect r)
{
...
bool SmartcardMod::is_resizing_hosted_desktop_allowed() const
{
...
void SmartcardMod::send_to_mod_channel(CHANNELS::ChannelNameId front_channel_name, InStream& chunk, size_t length, uint32_t flags)
{
    if (this->rail_enabled && this->rail_client_execute.is_ready()){
    ...
    if (front_channel_name == CHANNELS::channel_names::rdpdr) {
        this->manager->process_incoming_message(length, flags, chunk.remaining_bytes());
    }
}