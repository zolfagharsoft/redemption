...
{
    SCARD_PACK_CHECK_STREAM(stream, this, ERR_SCARD);
...
void scard_pack_call_establish_context::log(int level) const
{
    ...
    LOG(level, "dwScope: %" PRIu32, _dwScope);
    LOG(level, "}");
}
...
void scard_pack_return_establish_context::log(int level) const
{
    ...
    LOG(level, "ReturnCode: %" PRId32, _ReturnCode);
    LOG(level, "hContext: %l" PRId32, _Context.to_native());
    LOG(level, "}");
}
...
scard_pack_call_connect::scard_pack_call_connect(SCARDCONTEXT hContext,
...
void scard_pack_call_connect::log(int level) const
{
    ...
    LOG(level, "szReader: %s", (_szReader.is_nil() ? "(nil)" : bytes_to_hex_string(_szReader.data()).c_str()));
    LOG(level, "hContext: %l" PRId32, _Context.to_native());
    LOG(level, "dwShareMode: %" PRIu32, _dwShareMode);
    LOG(level, "dwPreferredProtocols: %" PRIu32, _dwPreferredProtocols);
    LOG(level, "}");
}
...
void scard_pack_return_connect::log(int level) const
{
    ...
    LOG(level, "ReturnCode: %" PRId32, _ReturnCode);
    LOG(level, "hContext: %l" PRId32, _Context.to_native());
    LOG(level, "hCard: %l" PRId32, _Card.to_native());
    LOG(level, "dwActiveProtocol: %" PRIu32, _dwActiveProtocol);
    LOG(level, "}");
}
...
void scard_pack_call_reconnect::log(int level) const
{
    ...
    LOG(level, "dwShareMode: %" PRIu32, _dwShareMode);
    LOG(level, "dwPreferredProtocols: %" PRIu32, _dwPreferredProtocols);
    LOG(level, "dwInitialization: %" PRIu32, _dwInitialization);
    LOG(level, "}");
}
...
void scard_pack_return_status::log(int level) const
{
    ...
    LOG(level, "dwState: %" PRIu32, _dwState);
...
void scard_pack_call_get_status_change::log(int level) const
{
    ...
    {
        LOG(level, "- %s", reader_state.repr().c_str());
    }
    LOG(level, "]");
    LOG(level, "}");
}
...
void scard_pack_call_control::log(int level) const
{
    ...
    LOG(level, "hContext: %l" PRId32, _Context.to_native());
    LOG(level, "hCard: %l" PRId32, _Card.to_native());
    LOG(level, "dwControlCode: %" PRIu32, _dwControlCode);
...
std::size_t scard_pack_call_transmit::pack(OutStream &stream) const
{
    ...
    // pioRecvPci (deferred)
    n_packed += _pioRecvPci.pack_elements(stream, 1, pointer_index);
...
const scard_pack_pci & scard_pack_return_transmit::ioRecvPci() const
{
    return _pioRecvPci.at(0);
}
...
void scard_pack_return_list_reader_groups::log(int level) const
{
    ...
    LOG(level, "ReturnCode: %" PRId32, _ReturnCode);
    LOG(level, "cBytes: %" PRIu32, _cBytes);
    LOG(level, "mszGroups: %s", (_mszGroups.empty() ? "(nil)" : bytes_to_hex_string(_mszGroups.data()).c_str()));
    LOG(level, "}");
}
...
void scard_pack_return_list_readers::log(int level) const
{
    ...
    LOG(level, "ReturnCode: %" PRId32, _ReturnCode);
    LOG(level, "cBytes: %" PRIu32, _cBytes);
    LOG(level, "mszReaders: %s", (_mszReaders.empty() ? "(nil)" : bytes_to_hex_string(_mszReaders.data()).c_str()));
    LOG(level, "}");
}
...
void scard_pack_call_get_attrib::log(int level) const
{
    ...
    LOG(level, "hContext: %l" PRId32, _Context.to_native());
    LOG(level, "hCard: %l" PRId32, _Card.to_native());
    LOG(level, "dwAttrId: %" PRIu32, _dwAttrId);
...
void scard_pack_return_get_attrib::log(int level) const
{
    ...
    LOG(level, "ReturnCode: %" PRId32, _ReturnCode);
    LOG(level, "cbAttrLen: %" PRIu32, _cbAttrLen);
    LOG(level, "pbAttr: %s", (_pbAttr.empty() ? "(nil)" : bytes_to_hex_string(_pbAttr.data()).c_str()));
    LOG(level, "}");
}
...
std::size_t pad(OutStream &stream, const std::size_t data_size)
{
...
std::size_t unpad(InStream &stream, const std::size_t data_size)
{
    std::size_t padding_size;
...
std::size_t get_padding_size(const std::size_t data_size)
{
    return ((data_size % 8) ? (8 - (data_size % 8)) : 0);
}

void check_stream(const InStream &stream, const scard_pack_return *return_,
...
void check_stream(const OutStream &stream, const scard_pack_call *call,
    const char */*file*/, const char *function, int line, error_type eid)
{
    assert(call);
