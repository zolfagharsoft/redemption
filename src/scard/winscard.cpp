...
static scard_pcsc_client    _client("/tmp/rdpcsc.socket", 0);
...
static void transmit(const scard_pack_call &call, scard_pack_return &return_) noexcept
{
    ...
    {
        // receive
        const auto buffer = _client.receive(call.io_control_code());
    ...
    LPDWORD pcbRecvLength)
{
    ...
    if (pbRecvBuffer)
    {
        assert(return_.pbRecvBuffer());

        std::memcpy(pbRecvBuffer, return_.pbRecvBuffer(),
            return_.cbRecvLength());
    }
    if (pcbRecvLength)
    {
